...
class OnePointThreeAcres:
	...
	def daily_checkin(self):
		if not self.solver:
			print("No captcha solver available, trying direct checkin...")
			# 尝试不解验证码直接签到
	...
	def get_daily_task_answer(self) -> tuple[int, int]:
		...
		if question in questions.keys():
			...
			if answer_id == "":
				...
				print("欢迎提交 PR 更新问题到 question.py")
		else:
	...
	def answer_daily_question(self, question: int, answer: int) -> bool:
		if not self.solver:
			print("No captcha solver available, trying direct answer...")
			body = {
				"qid": question,
				"answer": answer,
				"captcha_response": "",
		...
		if "人机验证出错，请重试" in response.text:
			...
			if self.solver and 'captcha_id' in locals():
		...
		if (result["errno"] == 0):
			...
			print("You have already answered the question today")
			return True
		else:
			print(response.text)
			return False
...
if __name__ == "__main__":
	# Set your cookie and api key
	cookie = """XXXXXXX"""
	TwoCaptcha_apikey = "xxxxxxxxxx"

	try:
		if not cookie or cookie == "XXXXXXX":
			raise ValueError("Environment variable cookie is not set")
		
		# initialize the solver (可以为空，如果没有验证码需求)
		solver = None
		if TwoCaptcha_apikey and TwoCaptcha_apikey != "xxxxxxxxxx":
			solver = TwoCaptcha(TwoCaptcha_apikey)
			print("2Captcha solver initialized")
		else:
			print("No 2Captcha API key provided, will try without captcha solving")
		...
		if not daily_checkin_status:
			print("Fail to check in the 1point3acres")
		# daily question
